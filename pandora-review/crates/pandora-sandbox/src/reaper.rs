...
impl ContainerReaper {
    ...
    pub async fn track(&self, container_id: String) {
        let mut tracking = self.tracked_containers.write().await;

        tracking.insert(container_id);
    }

    pub async fn untrack(&self, container_id: &str) {
        let mut tracking = self.tracked_containers.write().await;

        tracking.remove(container_id);
    }

    pub fn spawn_reaper_task(&self) {
    ...
    async fn reap_orphans(&self) {
        let tracked = {
            let lock = self.tracked_containers.read().await;
        ...
        info!("reaper analyzing {} tracked containers", tracked.len());
