...
Current candidate: v0.5.1-rc.5. Pandora has local release checks, installer scripts, API authentication, and desktop CI definitions. Mobile and Termux packaging are paused. This repository does not claim a production release yet. Stable release status remains blocked until GitHub publishes signed artifacts and clean-machine installation, upgrade, and removal tests pass for each supported desktop and CLI platform.
