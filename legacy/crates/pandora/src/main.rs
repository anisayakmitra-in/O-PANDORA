...
fn cmd_sessions(_args: &[String]) { let dir = sessions_dir(); if !dir.exists() { println!("No sessions yet."); return; } let mut s: Vec<pandora_types::Session> = Vec::new(); if let Ok(e) = std::fs::read_dir(&dir) { for entry in e.flatten() { let p = entry.path(); if p.extension().is_some_and(|e| e == "json") && p.file_stem() != Some(std::ffi::OsStr::new("index")) { if let Ok(j) = std::fs::read_to_string(&p) { if let Ok(ss) = serde_json::from_str::<pandora_types::Session>(&j) { s.push(ss); } } } } } s.sort_by(|a, b| b.created_at.cmp(&a.created_at)); println!("Sessions ({}):", s.len()); for ss in s.iter().take(10) { let st = match ss.status { pandora_types::SessionStatus::Completed => "ok", pandora_types::SessionStatus::Failed(_) => "err", _ => "?" }; println!("  {st} {}: {}", ss.id, &ss.prompt.chars().take(60).collect::<String>()); } }
