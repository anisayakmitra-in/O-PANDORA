...
impl ArtifactGraph {
    ...
    pub fn record(
        ...
        let short = if execution_id.len() > 12 {
            &execution_id[5..12]
        } else {
            execution_id
        };
    ...
    pub fn link(&mut self, parent_id: &str, child_id: &str) {
        if let Some(parent) = self.artifacts.get_mut(parent_id) {
            parent.derived.push(child_id.into());
        }
        if let Some(child) = self.artifacts.get_mut(child_id) {
            child.depends_on.push(parent_id.into());
        }
    }

    /// Get all artifacts for an execution.
    pub fn for_execution(&self, execution_id: &str) -> Vec<&Artifact> {
        self.artifacts
            .values()
    ...
    pub fn roots(&self) -> Vec<&Artifact> {
        self.artifacts
            .values()
    ...
    pub fn render(&self, execution_id: &str) -> String {
        ...
        if arts.is_empty() {
            return format!("No artifacts for: {execution_id}");
        }
        ...
        for art in &arts {
            out.push_str(&format!(
                ...
                art.label,
                art.size_bytes,
                art.content_hash.chars().take(12).collect::<String>()
            ));
            for dep in &art.depends_on {
                out.push_str(&format!("    depends on: {dep}\n"));
            }
            for der in &art.derived {
                out.push_str(&format!("    derived: {der}\n"));
            }
        }
        out
    }
