...
pub enum LifecycleState {
    Initialize, Plan, Execute, Verify, Recover, Complete, Abort,
}

impl LifecycleState {
    pub fn label(&self) -> &'static str {
        match self {
            Self::Initialize => "initialize", Self::Plan => "plan",
            Self::Execute => "execute", Self::Verify => "verify",
            Self::Recover => "recover", Self::Complete => "complete", Self::Abort => "abort",
        }
    }
    ...
    pub fn is_terminal(&self) -> bool { matches!(self, Self::Complete | Self::Abort) }
}
...
impl Lifecycle {
    pub fn new(id: &str, name: &str) -> Self {
        Self { id: id.into(), name: name.into(), state: LifecycleState::Initialize,
            steps: vec![], middleware: vec![], created_at: SystemTime::now(), completed_at: None }
    }
    pub fn transition(&mut self, next: LifecycleState) -> Result<(), String> {
        if !self.state.next_states().contains(&next) {
            return Err(format!("Invalid transition: {} → {}", self.state.label(), next.label()));
        }
        self.state = next;
        if next.is_terminal() { self.completed_at = Some(SystemTime::now()); }
        Ok(())
    }
    pub fn step(&mut self, name: &str, max_retries: u32) {
        self.steps.push(LifecycleStep { name: name.into(), state: self.state,
            started_at: Some(SystemTime::now()), completed_at: None, error: None,
