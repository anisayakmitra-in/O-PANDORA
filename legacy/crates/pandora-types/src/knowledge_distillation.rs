...
impl KnowledgeGraph {
    ...
    pub fn add_cluster(&mut self, cluster: KnowledgeCluster) {
...
impl KnowledgeDistillationEngine {
    ...
    pub fn config(&self, tier: KnowledgeTier) -> Option<&DistillationConfig> {
    ...
    pub fn ingest(
        ...
        let event = KnowledgeEvent::NodeCreated {
    ...
    pub fn query(&self, q: &KnowledgeQuery) -> Vec<&KnowledgeNode> {
        let mut results: Vec<&KnowledgeNode> = self
            .graph
            .nodes
            .iter()
            .filter(|n| {
                q.tier.is_none_or(|t| n.tier == t)
                    ...
                    && (q.session_ids.is_empty()
                        || q.session_ids.iter().any(|s| n.source_sessions.contains(s)))
