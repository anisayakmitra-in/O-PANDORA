        ...
        items.into_iter().map(|item| {
            ...
            if spent + cost > self.max_budget {
                WorkResult {
                    item_id: item.id,
                    success: false,
                    output: format!("Skipped: budget exceeded (spent {spent:.2}, cap {:.2})", self.max_budget),
                    duration_ms: 0,
                }
            } else {
                spent += cost;
...
mod tests {
    ...
    fn sequential_runs_all() {
        ...
        let items = vec![
            WorkItem { id: "a".into(), priority: 1, deadline: None, budget: None, affinity: vec![], payload: "task-a".into() },
            WorkItem { id: "b".into(), priority: 1, deadline: None, budget: None, affinity: vec![], payload: "task-b".into() },
        ];
    ...
    fn priority_sorts_by_priority() {
        ...
        let items = vec![
            WorkItem { id: "low".into(), priority: 1, deadline: None, budget: None, affinity: vec![], payload: "low".into() },
            WorkItem { id: "high".into(), priority: 100, deadline: None, budget: None, affinity: vec![], payload: "high".into() },
        ];
        let results = s.schedule(items);
    ...
    fn budget_enforces_cap() {
        ...
        let items = vec![
            WorkItem { id: "cheap".into(), priority: 1, deadline: None, budget: Some(2.0), affinity: vec![], payload: "cheap".into() },
            WorkItem { id: "expensive".into(), priority: 1, deadline: None, budget: Some(10.0), affinity: vec![], payload: "expensive".into() },
        ];
        let results = s.schedule(items);
