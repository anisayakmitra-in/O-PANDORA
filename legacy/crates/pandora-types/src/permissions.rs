...
impl Permission {
    pub fn name(&self) -> &'static str {
        match self {
            ...
            Self::FilesystemExec => "filesystem.exec",
            Self::ShellExecute => "shell.execute",
            Self::ShellPipe => "shell.pipe",
            Self::NetworkHttp => "network.http",
            Self::NetworkRaw => "network.raw",
            Self::NetworkBind => "network.bind",
            Self::ProviderRemote => "provider.remote",
            Self::ProviderLocal => "provider.local",
            Self::MemoryRead => "memory.read",
            Self::MemoryWrite => "memory.write",
            Self::SandboxRestricted => "sandbox.restricted",
            Self::SandboxIsolated => "sandbox.isolated",
            Self::RuntimeEvents => "runtime.events",
            Self::RuntimeSessions => "runtime.sessions",
            Self::RuntimePlans => "runtime.plans",
            Self::RuntimeProviders => "runtime.providers",
            Self::RuntimeMarketplace => "runtime.marketplace",
        }
    }
}
...
impl PermissionSet {
    ...
    pub fn minimal() -> Self {
        Self {
            permissions: vec![Permission::FilesystemRead, Permission::ProviderLocal],
        }
    }
    ...
    pub fn full() -> Self {
        Self {
            permissions: vec![
                ...
                Permission::FilesystemExec,
                Permission::ShellExecute,
                Permission::ShellPipe,
                Permission::NetworkHttp,
                Permission::NetworkRaw,
                Permission::NetworkBind,
                Permission::ProviderRemote,
                Permission::ProviderLocal,
                Permission::MemoryRead,
                Permission::MemoryWrite,
                Permission::SandboxRestricted,
                Permission::SandboxIsolated,
            ],
        }
    }
        ...
        for p in &self.permissions {
            match p {
                Permission::ShellExecute | Permission::ShellPipe => {
                    if sandbox_level == 0 {
                        violations.push(format!("{} requires sandbox", p.name()));
                    }
                }
                Permission::NetworkRaw | Permission::NetworkBind if sandbox_level < 2 => {
                    violations.push(format!("{} requires isolated sandbox", p.name()));
                }
                _ => {}
            }
        }
...
mod tests {
    ...
    #[test]
    fn minimal_has_read() {
        assert!(PermissionSet::minimal().has(&Permission::FilesystemRead));
    }
    #[test]
    fn full_has_all() {
        assert_eq!(PermissionSet::full().permissions.len(), 14);
    }
    #[test]
    fn compatible_sandbox() {
        assert!(PermissionSet::minimal().compatible_with(0).is_ok());
    }
    #[test]
    fn shell_requires_sandbox() {
        assert!(PermissionSet::full().compatible_with(0).is_err());
    }
}
