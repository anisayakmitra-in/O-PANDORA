...
impl ProviderDb {
    ...
    pub fn record(&mut self, obs: ProviderObservation) {
        ...
        let tps = if obs.latency_ms > 0 {
            obs.tokens_used as f64 / obs.latency_ms as f64 * 1000.0
        } else {
            0.0
        };
        ...
        entry.success_rate =
            (entry.success_rate * n + if obs.success { 1.0 } else { 0.0 }) / (n + 1.0);
        entry.last_seen = obs.timestamp;
        if obs.success {
            entry.last_healthy = Some(obs.timestamp);
        }
        self.observations.push(obs);
    }

    /// Get metrics for a specific provider+model.
    pub fn metrics(&self, provider: &str, model: &str) -> Option<&ProviderMetrics> {
        self.metrics.get(&(provider.into(), model.into()))
    }
    ...
    pub fn best(&self, criteria: &str) -> Option<(&str, &str)> {
        self.metrics
            .iter()
            .filter(|(_, m)| m.total_calls > 0)
            .max_by(|(_, a), (_, b)| match criteria {
...
mod tests {
    ...
    fn record_and_metrics() {
        let mut db = ProviderDb::new();
        db.record(ProviderObservation {
        ...
        let m = db.metrics("ollama", "qwen").unwrap();
        assert_eq!(m.total_calls, 1);
        assert!(m.avg_tokens_per_sec > 0.0);
    }
    #[test]
    fn best_by_speed() {
        let mut db = ProviderDb::new();
        ...
        let best = db.best("fastest");
        assert!(best.is_some());
        assert_eq!(best.unwrap().0, "ollama");
    }
    #[test]
    fn empty_db() {
        let db = ProviderDb::new();
        assert!(db.is_empty());
        assert!(db.best("fastest").is_none());
    }
}
