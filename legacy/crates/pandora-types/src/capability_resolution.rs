...
impl CapabilityResolutionEngine {
    ...
    pub fn resolve_domain(&self, domain: &str) -> Vec<CapabilityCandidate> {
        self.resolve(&CapabilityRequest {
            domain: domain.into(),
    ...
    fn meets_constraints(&self, entry: &ProviderEntry, c: &CapabilityConstraints) -> bool {
        ...
        if let Some(max) = c.max_latency_ms {
            if entry.latency_ms > max as f64 {
                return false;
            }
        }
        if c.require_tools && !entry.supports_tools {
            return false;
        }
        if c.require_vision && !entry.supports_vision {
            return false;
        }
        if let Some(min) = c.min_context {
            if entry.context_limit < min {
                return false;
            }
        }
        true
