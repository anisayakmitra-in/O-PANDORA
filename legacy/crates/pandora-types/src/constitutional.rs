...
impl ManifestRelationshipSet {
    ...
    pub fn push(mut self, r: ManifestRelationship) -> Self {
...
impl ConstitutionalManifestBuilder {
    pub fn new(name: impl Into<String>, kind: ManifestKind, version: ManifestVersion) -> Self {
        Self {
            inner: ConstitutionalManifest::new(name, kind, version, ""),
        }
    }
...
impl InMemoryManifestLoader {
    ...
    pub fn push(mut self, m: ConstitutionalManifest) -> Self {
...
impl ManifestRegistry {
    ...
    pub fn register(&self, m: ConstitutionalManifest) {
        self.inner.write().expect("registry poisoned").push(m);
    }
    pub fn unregister(&self, name: &str) -> bool {
        let mut g = self.inner.write().expect("registry poisoned");
        let b = g.len();
        g.retain(|m| m.identity.name != name);
        g.len() != b
    }
    ...
    pub fn validate(&self) -> Vec<(String, ManifestValidationError)> {
        ...
        g.iter()
            .filter_map(|m| v.validate(m).err().map(|e| (m.identity.name.clone(), e)))
            .collect()
    }
...
mod tests {
    ...
    fn manifest_identity_id() {
        assert_eq!(fixture().id(), "phoenix@1.0.0");
    }
    #[test]
    fn manifest_version_string() {
        assert_eq!(ManifestVersion::new(1, 2, 3).as_string(), "1.2.3");
    }
    #[test]
    fn manifest_kind_names() {
        assert_eq!(ManifestKind::SourceHarness.name(), "SourceHarness");
        assert_eq!(ManifestKind::Custom("x".into()).name(), "x");
    }
    #[test]
    fn json_roundtrip() {
        let m = fixture();
        let s = JsonManifestSerializer::new().serialize(&m).unwrap();
        assert_eq!(m, JsonManifestSerializer::new().deserialize(&s).unwrap());
    }
    #[test]
    fn validator_accepts_valid() {
        assert!(ManifestValidator::new().validate(&fixture()).is_ok());
    }
    #[test]
    fn validator_rejects_empty_name() {
        let mut m = fixture();
        m.identity.name = "   ".into();
        assert!(ManifestValidator::new().validate(&m).is_err());
    }
    #[test]
    fn validator_rejects_zero_major() {
        let mut m = fixture();
    ...
    fn registry_register_and_lookup() {
        ...
        let m = fixture();
        r.register(m.clone());
        assert_eq!(r.lookup("phoenix"), Some(m));
    }
    #[test]
    ...
    #[test]
    fn yaml_unavailable() {
        assert!(YamlManifestSerializer::new().serialize(&fixture()).is_err());
    }
    #[test]
    fn toml_unavailable() {
        assert!(TomlManifestSerializer::new().serialize(&fixture()).is_err());
    }
}
