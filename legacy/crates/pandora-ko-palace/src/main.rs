//! Pandora K-O-Palace — CLI entry point.

use pandora_ko_palace::KoPalace;
use pandora_shadow_council::ShadowCouncil;
use std::env;
use std::process;
use std::sync::{Arc, RwLock};

fn get_ko_palace() -> KoPalace {
    let sc = Arc::new(RwLock::new(ShadowCouncil::new()));
    KoPalace::new(sc.clone())
}

fn main() {
    let args: Vec<String> = env::args().collect();
    if args.len() < 2 {
        eprintln!("Pandora K-O-Palace — gene distribution system\n");
        eprintln!("Usage: pandora-ko-palace <command> [options]\n");
        eprintln!("Commands:");
...
fn cmd_install(args: &[String]) -> Result<(), pandora_types::PandoraError> {
    if args.len() < 3 {
        return Err("Usage: pandora-ko-palace install <id>".into());
    }
    let mut ko_palace = get_ko_palace();
    if let Ok(cwd) = env::current_dir() {
        ko_palace.add_source("local", &cwd.to_string_lossy());
    }
    ko_palace.install(&args[2])?;
...
fn cmd_uninstall(args: &[String]) -> Result<(), pandora_types::PandoraError> {
    if args.len() < 3 {
        return Err("Usage: pandora-ko-palace uninstall <id>".into());
    }
    let mut ko_palace = get_ko_palace();
    ko_palace.uninstall(&args[2])?;
...
fn cmd_search(args: &[String]) -> Result<(), pandora_types::PandoraError> {
    if args.len() < 3 {
        return Err("Usage: pandora-ko-palace search <query>".into());
    }
    let ko_palace = get_ko_palace();
    let results = ko_palace.search(&args[2]);
    if results.is_empty() {
...
fn cmd_info(args: &[String]) -> Result<(), pandora_types::PandoraError> {
    if args.len() < 3 {
        return Err("Usage: pandora-ko-palace info <id>".into());
    }
    let ko_palace = get_ko_palace();
    match ko_palace.info(&args[2]) {
        Some(p) => {
...
fn cmd_list() -> Result<(), pandora_types::PandoraError> {
    let ko_palace = get_ko_palace();
    let list = ko_palace.list_installed();
    if list.is_empty() {
...
fn cmd_available() -> Result<(), pandora_types::PandoraError> {
    let ko_palace = get_ko_palace();
    let list = ko_palace.list_available();
    if list.is_empty() {
...
fn cmd_source(args: &[String]) -> Result<(), pandora_types::PandoraError> {
    if args.len() < 3 {
        return Err("Usage: pandora-ko-palace source <add|remove|list> [...]".into());
    }
    let mut ko_palace = get_ko_palace();
    match args[2].as_str() {
        "add" => {
            if args.len() < 5 {
                return Err("Usage: pandora-ko-palace source add <name> <path>".into());
            }
            ko_palace.add_source(&args[3], &args[4]);
            println!("Added source: {} -> {}", args[3], args[4]);
        }
        "remove" => {
            if args.len() < 4 {
                return Err("Usage: pandora-ko-palace source remove <name>".into());
            }
            ko_palace.remove_source(&args[3]);
            println!("Removed source: {}", args[3]);
        }
        "list" => {
            let sources = ko_palace.list_sources();
            if sources.is_empty() {
                println!("No sources.");
            } else {
                for s in sources {
...
fn cmd_skill(args: &[String]) -> Result<(), pandora_types::PandoraError> {
    if args.len() < 3 {
        return Err("Usage: pandora-ko-palace skill <install|scaffold|list> [...]".into());
    }
    match args[2].as_str() {
        "install" => {
            if args.len() < 4 {
                return Err("Usage: pandora-ko-palace skill install <path>".into());
            }
            let mut ko_palace = get_ko_palace();
            if let Ok(cwd) = env::current_dir() {
                ko_palace.add_source("local", &cwd.to_string_lossy());
            }
            let skill = pandora_ko_palace::skill::install(&mut ko_palace, &args[3])?;
            println!("Skill: {} v{}", skill.manifest.name, skill.manifest.version);
        }
        "scaffold" => {
            if args.len() < 4 {
                return Err("Usage: pandora-ko-palace skill scaffold <name> [dir]".into());
            }
            let dir = if args.len() > 4 { &args[4] } else { "." };
            let path = pandora_ko_palace::skill::scaffold(&args[3], dir)?;
            println!("Created: {}", path);
        }
        "list" => {
            let dir = if args.len() > 3 { &args[3] } else { "." };
            let skills = pandora_ko_palace::skill::discover(dir);
            if skills.is_empty() {
...
fn cmd_score(args: &[String]) -> Result<(), pandora_types::PandoraError> {
    if args.len() < 3 {
        return Err("Usage: pandora-ko-palace score <path>".into());
    }
    let ko_palace = get_ko_palace();
    let score = ko_palace.score(&args[2])?;
