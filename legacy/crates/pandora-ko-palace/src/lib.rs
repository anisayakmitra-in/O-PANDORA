//! Pandora K-O-Palace — gene distribution system.
...
impl KoPalace {
    ...
    pub fn install(&mut self, id: &str) -> Result<(), pandora_types::PandoraError> {
        ...
        if already_installed {
            println!(
                "[ko_palace] {id} v{} already installed",
        ...
        if lockfile_wiring::has_changed(&lock, id, &pkg.manifest.version) {
            println!(
                "[ko_palace] version change detected for {id}: {} -> {}",
        ...
        if !resolved_lock.is_empty() {
            ...
            println!(
                "[ko_palace] lockfile updated with {} entries",
    ...
    pub fn upgrade(&mut self, id: &str) -> Result<(), pandora_types::PandoraError> {
        ...
        match &action {
            upgrade::UpgradeAction::UpToDate { version } => {
                println!("[ko_palace] {id} v{version} is already up to date");
                return Ok(());
            }
            upgrade::UpgradeAction::Upgrade { from, to } => {
                println!("[ko_palace] upgrading {id}: {from} -> {to}");
            }
            upgrade::UpgradeAction::Downgrade { from, to } => {
                println!("[ko_palace] downgrading {id}: {from} -> {to}");
            }
        }
        ...
        if install_dir.exists() {
            match upgrade::backup_package(id, &install_dir) {
                Ok(bak) => println!("[ko_palace] backed up to {}", bak.display()),
                Err(e) => {
                    println!("[ko_palace] warning: backup failed: {e}");
                }
            }
        }
        ...
        match self.install(id) {
            Ok(()) => {
                println!("[ko_palace] {id} upgraded successfully");
                Ok(())
            }
            Err(e) => {
                println!("[ko_palace] upgrade failed: {e}, attempting rollback...");
                if let Err(rb_err) = upgrade::rollback_package(id, &install_dir) {
                    Err(to_err(format!(
                        "Upgrade failed ({e}) and rollback also failed ({rb_err})"
                    )))
                } else {
                    println!("[ko_palace] rolled back to previous version");
                    self.council_write()
...
mod tests {
    ...
    fn manage_sources() {
        ...
        let mut k = KoPalace::new(sc.clone());
    ...
    fn scoring_works() {
        ...
        let k = KoPalace::new(sc.clone());
        let d = tmp();
        std::fs::create_dir_all(d.join("g1").join("src")).expect("ko_palace");
        std::fs::write(
        ...
        .expect("ko_palace");
        let s = k.score(d.to_str().expect("ko_palace")).expect("ko_palace");
        assert!(s.overall() >= 5);
        assert!(s.tests >= 5);
        std::fs::remove_dir_all(d).expect("ko_palace");
    }
    ...
    fn skill_scaffold() {
        let d = tmp();
        let path = crate::skill::scaffold("test-skill", d.to_str().expect("ko_palace"))
            .expect("ko_palace");
    ...
    fn skill_discover_empty() {
        let d = tmp();
        std::fs::create_dir_all(&d).expect("ko_palace");
        assert!(crate::skill::discover(d.to_str().expect("ko_palace")).is_empty());
        std::fs::remove_dir_all(d).expect("ko_palace");
    }
...
"#,
    ...
    #[test]
    fn ko_palace_installed_count_zero() {
        let sc = Arc::new(RwLock::new(ShadowCouncil::new()));
        let k = KoPalace::new(sc);
        assert_eq!(k.installed_count(), 0);
    }

    #[test]
    fn ko_palace_search_empty() {
        let sc = Arc::new(RwLock::new(ShadowCouncil::new()));
        let k = KoPalace::new(sc);
