...
async fn health() -> impl IntoResponse {
    Json(serde_json::json!({"status": "ok", "service": "pandora-palace", "version": "0.1.0"}))
}
...
async fn search_packages(
    ...
    let results: Vec<PalaceListing> = {
        state
            ...
            .map(|p| PalaceListing {
                full_id: package_id(&p.publisher, &p.manifest.id),
                ...
                success_rate: p.manifest.success_rate,
                ..Default::default()
            })
            .collect()
    };
    ...
    let user = match authenticate(&state, &headers) {
        ...
        None => {
            return (
                ...
                Json(ApiError {
                    code: 401,
                    message: "Authentication required. Use: pandora login".into(),
                }),
            )
                .into_response()
        }
    ...
    if !req.manifest.publisher.is_empty() && req.manifest.publisher != user.username {
        return (
            ...
            Json(ApiError {
                code: 403,
                message: format!(
                    "Publisher {} does not match authenticated user {}",
                    req.manifest.publisher, user.username
                ),
            }),
        )
            .into_response();
    }
    let publisher_name = if req.manifest.publisher.is_empty() {
        user.username.clone()
    } else {
        req.manifest.publisher.clone()
    };
...
async fn login(
    State(state): State<PalaceState>,
    Json(creds): Json<serde_json::Value>,
) -> impl IntoResponse {
    ...
    let user = users.entry(username.to_string()).or_insert(PalaceUser {
        ...
        email: format!("{username}@palace.local"),
    ...
    // Seed some demo packages
    {
        ...
        for (id, name, kind, desc, author) in &[
            ...
            (
                ...
                "Rust Backend Skill",
                "skill",
                "Complete Rust backend development skill",
                "pandora",
            ),
        ] {
            pkgs.insert(
                ...
                RegistryPackage {
                    manifest: PackageManifest {
                        publisher: "pandora".into(),
                        id: id.to_string(),
                        name: name.to_string(),
                        version: "0.1.0".into(),
                        kind: match *kind {
                            "gene" => PackageKind::Gene,
                            "domain-harness" => PackageKind::DomainHarness,
                            "evaluator" => PackageKind::Evaluator,
                            "skill" => PackageKind::Skill,
                            _ => PackageKind::Bundle,
                        },
                        description: desc.to_string(),
                        author: author.to_string(),
                        pandora_version: ">=1.0".into(),
