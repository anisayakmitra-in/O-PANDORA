...
pub fn discover(root: &str) -> Vec<SkillManifest> {
    let mut r = Vec::new();
    if let Ok(dir) = std::fs::read_dir(root) { for entry in dir.flatten() {
        let p = entry.path();
        if p.is_dir() && p.join("skill.toml").exists() { if let Ok(c) = std::fs::read_to_string(p.join("skill.toml")) { if let Ok(m) = toml::from_str(&c) { r.push(m); } } }
    }}
    r
}
...
pub fn install(kuber: &mut Kuber, path: &str) -> Result<Skill, String> {
    ...
    let manifest: SkillManifest = toml::from_str(&content).map_err(|e| format!("Invalid skill.toml: {e}"))?;
    for gene in &manifest.genes { match kuber.install(&gene.id) { Ok(_) => println!("  [gene] {}", gene.id), Err(e) => eprintln!("  [warn] {}: {e}", gene.id) } }
