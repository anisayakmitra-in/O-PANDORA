...
impl SecurityDomainHarness {
    pub fn new() -> Self {
        Self {
            manifest: HarnessManifestBuilder::default()
                .id("security-domain").name("Security").version("0.1.0").author("pandora").kind(HarnessKind::Domain)
                .description("Security analysis — audit, scan, secrets, threat model")
                .capability("security-audit").capability("dependency-scan")
                .capability("secrets-detection").capability("static-analysis")
    ...
    pub fn audit(&self, dir: &str) -> Result<String, String> {
        run("cargo", &["audit", "--manifest-path", &format!("{dir}/Cargo.toml")])
...
impl Default for SecurityDomainHarness { fn default() -> Self { Self::new() } }
