...
android_gene!(
    AndroidIntentGene,
    "android-intent",
    "Broadcast Android intents"
);
android_gene!(
    AndroidPlanGene,
    "android-plan",
    "Self-planning: decompose compound commands (OpenDroid)"
);
android_gene!(
    AndroidReflectGene,
    "android-reflect",
    "Re-evaluate after failure, replan (OpenDroid)"
);
...
impl CompoundIntentDetector {
    ...
    pub fn detect(task: &str) -> Vec<String> {
        ...
        for m in &markers {
            parts = parts
                .iter()
                .flat_map(|p| p.split(m).map(String::from))
                .collect();
        }
        parts
            .into_iter()
...
mod tests {
    ...
    #[test]
    fn harness_id() {
        assert_eq!(AndroidUseHarness::new().manifest().id, "android-use");
    }
    #[test]
