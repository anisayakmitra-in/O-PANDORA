...
impl ComputerUseHarness {
    pub fn new() -> Self {
        Self { manifest: HarnessManifestBuilder::default()
            .id("computer-use").name("Computer Use").version("0.1.0").author("pandora")
            .kind(HarnessKind::Domain)
            .description("Desktop automation — click, type, screenshot, find elements")
            .capability("screenshot").capability("click").capability("typing").capability("context")
            .build().unwrap() }
    }
}
    ...
    ($name:ident, $id:expr, $desc:expr) => {
        ...
        impl Gene for $name {
            ...
            fn execute(&self, _input: &str) -> Result<String, String> {
                let os = platform();
                let t = match os { "linux" => "xdotool/imagemagick", "macos" => "osascript/screencapture", _ => "not available" };
...
ghost_gene!(GhostContextGene, "ghost-context", "Focused app, window, URL, interactive elements");
ghost_gene!(GhostClickGene, "ghost-click", "Click coordinates or element. AX-native first, synthetic fallback");
ghost_gene!(GhostTypeGene, "ghost-type", "Type text. Focus the target field first with ghost_click");
ghost_gene!(GhostScreenshotGene, "ghost-screenshot", "Capture screen to PNG file");
ghost_gene!(GhostFindGene, "ghost-find", "Find elements by query, role, or ID");
ghost_gene!(GhostReadGene, "ghost-read", "Read text from screen region");
ghost_gene!(GhostScrollGene, "ghost-scroll", "Scroll in a window or element");
ghost_gene!(GhostHotkeyGene, "ghost-hotkey", "Press keyboard shortcut");
ghost_gene!(GhostInspectGene, "ghost-inspect", "Full metadata about one element");

#[cfg(test)]
mod tests {
    use super::*;
    #[test] fn harness_id() { assert_eq!(ComputerUseHarness::new().manifest().id, "computer-use"); }
    #[test] fn gene_id() { assert_eq!(GhostClickGene::new().manifest().id, "ghost-click"); }
}
