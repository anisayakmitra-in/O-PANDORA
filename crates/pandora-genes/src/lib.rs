...
impl Gene for DockerfileEvaluator {
    ...
    fn execute(&self, input: &str) -> Result<String, String> {
        ...
        std::process::Command::new(&docker).args(&["build", "-q", input]).output()
...
impl Gene for ShellCheckEvaluator {
    ...
    fn manifest(&self) -> &GeneManifest { &self.m }
    fn execute(&self, input: &str) -> Result<String, String> {
        std::process::Command::new("shellcheck").args(input.split_whitespace().collect::<Vec<_>>()).output()
...
impl Gene for MarkdownLintEvaluator {
    ...
    fn manifest(&self) -> &GeneManifest { &self.m }
    fn execute(&self, input: &str) -> Result<String, String> {
        let result = std::process::Command::new("markdownlint").args(input.split_whitespace().collect::<Vec<_>>()).output();
        match result {
            Ok(o) if o.status.success() => Ok("Markdown: no issues".into()),
            Ok(o) => Err(String::from_utf8_lossy(&o.stderr).to_string()),
            Err(_) => {
                // Fallback to mdl
                std::process::Command::new("mdl").args(input.split_whitespace().collect::<Vec<_>>()).output()
