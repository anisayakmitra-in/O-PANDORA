...
impl ConstitutionalReliabilityBenchmarkEngine {
    pub fn benchmark(signals: &[BenchmarkSignal]) -> BenchmarkState {
        ...
        for signal in signals {
            println!("[BENCHMARK] benchmark={}", signal.benchmark_id);
            ...
            let governance_certified = signal.governance_stability > 0.84;

            let replay_certified = signal.replay_integrity > 0.82;

            let mutation_promotion_allowed = signal.mutation_survivability > 0.86;

            let autonomy_expansion_allowed = signal.autonomy_stability > 0.88;
        ...
        let constitutional_reliability = reliability / count;

        let replay_stability = replay / count;

        let governance_survivability = governance / count;

        let sovereign_benchmark_stable = constitutional_reliability > 0.84
            && replay_stability > 0.83
            && governance_survivability > 0.84;
