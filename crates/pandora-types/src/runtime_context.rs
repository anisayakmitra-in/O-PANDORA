//! Constitutional Execution Runtime (CER) — Runtime Context and Execution Properties.
//!
//! RuntimeContext is the foundation for all execution. Every subsystem receives
//! &RuntimeContext instead of random parameters. It exists only while something runs.
//!
...
impl Default for ContextWindowStrategy {
    fn default() -> Self {
        ContextWindowStrategy::Fixed(128_000)
    }
}
...
///
/// Every subsystem receives &RuntimeContext.
/// It exists only while something runs.
/// It is NOT memory. Memory is persistence. Context is execution state.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    fn elapsed_time() {
        ...
        assert!(ctx.elapsed_secs() < 5); // Just created
    }
