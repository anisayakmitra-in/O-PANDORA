...
impl ExperimentEngine {
    ...
    pub fn analyze(&mut self, experiment_id: &str) -> Result<ExperimentReport, String> {
        ...
        for variant in &exp.variants {
            let variant_runs: Vec<&VariantRun> = self
                .runs
                .iter()
            ...
            let avg_latency =
                variant_runs.iter().map(|r| r.latency_ms).sum::<f64>() / scores.len() as f64;
        ...
        let recommendation = if winner_score > 80.0 {
            format!(
                "Strongly recommend '{}' (score: {:.1}). High confidence.",
                winner, winner_score
            )
        } else if winner_score > 60.0 {
            format!(
                "Recommend '{}' (score: {:.1}). Moderate confidence.",
                winner, winner_score
            )
        } else {
            format!(
                "No clear winner. Best variant '{}' scored {:.1}. Consider more runs.",
                winner, winner_score
            )
        };
